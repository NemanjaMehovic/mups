...
void par(int arc, char **argv, int rank, int sizeOfCluster)
{
  double a = 3.0;
  double b = 2.0;
  double c = 1.0;
    ...
    for (i = 1; i <= ni; i++)
    {
      ...
      for (j = 1; j <= nj; j++)
      {
        ...
        for (k = 1; k <= nk; k++)
        {
          ...
          steps = 0;
          work = 1;
          MPI_Bcast(&work, 1, MPI_INT, MASTER, MPI_COMM_WORLD);
          MPI_Bcast(&x, 1, MPI_DOUBLE, MASTER, MPI_COMM_WORLD);
          MPI_Bcast(&y, 1, MPI_DOUBLE, MASTER, MPI_COMM_WORLD);
          MPI_Bcast(&z, 1, MPI_DOUBLE, MASTER, MPI_COMM_WORLD);
          for (trial = rank; trial < N; trial+=sizeOfCluster)
            {
              seed = seed + rank;
              x1 = x;
              x2 = y;
          ...
          MPI_Reduce(&wt, &tmpWt, 1, MPI_DOUBLE, MPI_SUM, MASTER, MPI_COMM_WORLD);
          MPI_Reduce(&steps, &tmpSteps, 1, MPI_INT, MPI_SUM, MASTER, MPI_COMM_WORLD);

          wt = tmpWt / (double)(N);
          steps_ave = tmpSteps / (double)(N);
