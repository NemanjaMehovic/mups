...
void forces(int npart, double x[], double f[], double side, double rcoff, int rank, int sizeOfCluster, int i)
{
  int j, tmp;
  double sideh, rcoffs;
  ...
  rcoffs = rcoff * rcoff;
    ...
    for (j = i + 3; j < npart * 3; j += 3)
    {
      ...
      if (rd <= rcoffs)
      {
        ...
        data[2] = forcez;
        data[3] = j;
        MPI_Isend(data, 4, MPI_DOUBLE, MASTER, SUB, MPI_COMM_WORLD, &request);
        flagMsgSent = 1;
      }
    }
    if(flagMsgSent)
    ...
    data[4] = epot;
    data[5] = vir;
    MPI_Send(data, 6, MPI_DOUBLE, MASTER, ENDDATA, MPI_COMM_WORLD);
...
secnds(void);
    ...
    while(keepWorking)
    {
      ...
      while(flag)
      {
        ...
        switch(status.MPI_TAG)
        {
          ...
          break;
          case WORK:
          forces(npartt, x, f, side, rcoff, rank, sizeOfCluster, i);
          MPI_Send(NULL, 0, MPI_DOUBLE, MASTER, FIN, MPI_COMM_WORLD);
          break;
        }
      }
      MPI_Bcast(&keepWorking, 1, MPI_INT, MASTER, MPI_COMM_WORLD);
  ...
  void manager(int rank, int sizeOfCluster, int n, double f[])
  {
      MPI_Status status;
      int numOfWorkers = 0, i = 0, j;
      double buff[6];
      vir = 0.0;
      epot = 0.0;
      for(j = 1; j < sizeOfCluster && i < n; j++)
      {
          MPI_Send(&i, 1, MPI_INT, j, WORK, MPI_COMM_WORLD);
          numOfWorkers++;
          i += 3;
      }
      while( j < sizeOfCluster)
      {
         MPI_Send(NULL, 0, MPI_INT, j, WAIT, MPI_COMM_WORLD);
         j++;
      }
      while(i < n)
      {
        MPI_Recv(buff, 6, MPI_DOUBLE, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
        switch(status.MPI_TAG)
        {
          case FIN:
          MPI_Send(&i, 1, MPI_INT, status.MPI_SOURCE, WORK, MPI_COMM_WORLD);
          i += 3;
          break;
          case ENDDATA:
          j = buff[3];
      ...
      while(numOfWorkers > 0)
      {
        ...
        {
          case FIN:
          MPI_Send(NULL, 0, MPI_INT, status.MPI_SOURCE, WAIT, MPI_COMM_WORLD);
          numOfWorkers--;
          break;
          case ENDDATA:
          j = buff[3];
...
void par(int rank, int sizeOfCluster)
{
  ...
  for (move = 1; move <= movemx; move++)
  {
     ...
     */
     ...
     MPI_Bcast(&side, 1, MPI_DOUBLE, MASTER, MPI_COMM_WORLD);
     MPI_Bcast(&rcoff, 1, MPI_DOUBLE, MASTER, MPI_COMM_WORLD);
     manager(rank, sizeOfCluster, npartt*3, f);
