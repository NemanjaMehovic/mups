...
void par(int arc, char **argv)
{
  ...
  #pragma omp parallel \
  default(none) \
  private(x1, x2, x3, w, we, chk, us, ut , dx, dy, dz, vs, vh , i, j, k, x, y, z, w_exact, wt, steps, steps_ave) \
  shared(a, b, c, h, N, stepsz, err, n_inside, seeds) \
  firstprivate(seed, ni, nj, nk)
  {
    #pragma omp single
    {
      int numOfThreads = omp_get_num_threads();
      seeds = malloc(sizeof(int)*numOfThreads);
      ...
      for (i = 1; i <= ni; i++)
      {
        ...
        for (j = 1; j <= nj; j++)
        {
          ...
          for (k = 1; k <= nk; k++)
          {
            ...
            #pragma omp task \
            default(none) \
            private(x1, x2, x3, w, we, chk, us, ut , dx, dy, dz, vs, vh, wt, steps_ave, steps, trial) \
            shared(a, b, c, h, N, stepsz, err, seeds) \
            firstprivate(x, y, z, w_exact)
            {
              wt = 0.0;
              steps = 0;
